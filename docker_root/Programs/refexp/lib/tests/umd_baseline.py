...
parser.add_argument("-k", "--K", dest="K", help="top k matches sent to UMD")
...
def pub_image():
    ...
    with open(metadata_path, 'r') as m_file:
        ...
        for line in m_file:
            ...
            goal_loc = action_controller.msg.DenseRefexpLoadGoal(msg_frame)
            ...
            for (count, idx) in enumerate(context_boxes_idxs):
                ...
                if count == len(context_boxes_idxs)-1:
                    ...
                    cv2.rectangle(draw_img, (x1, y1), (x2, y2), (255,255,0), 1)

            path = save_path + '/' + image_name.replace('.png', '') + "_umd_result.png"
